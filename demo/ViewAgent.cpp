...
ViewAgent::ViewAgent(Ped::Tagent * agent, QGraphicsScene * scene, int id_input) : agent(agent)
{
	id = id_input;
...
void ViewAgent::paint(QColor color, Ped::Simd_funcs *simd){
	int x = (simd == NULL) ? agent->getX() : simd->getPositionX(id);
	int y = (simd == NULL) ? agent->getY() : simd->getPositionY(id);

	if(bgt_icon)
		bgt_icon->setPos(MainWindow::cellToPixel(x), MainWindow::cellToPixel(y));
	else
	{
		QBrush brush(color);
		rect->setBrush(brush);
		rect->setRect(MainWindow::cellToPixel(x), MainWindow::cellToPixel(y),
